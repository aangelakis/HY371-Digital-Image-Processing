...
% Exercise 1_a
% Sub-sampling and up-sampling using the Nearest Neighbour method
...
for k = 1:(length(myFiles))
  ...
  sub_sampled_img_my_imresize = my_imresize_NN(norm_img, a, 'nn_subsampling');
  %figure; imshow(sub_sampled_img_my_imresize); title(['Sub-sampled Image using my function: ', baseFileName], 'Interpreter', 'none');

  % Up-sampling the sub-sampled image using the imresize function and the my_imresize_NN and plotting it
  restored_img_imresize = imresize(sub_sampled_img_imresize, 1/a, method = "nearest");
  restored_img_my_imresize = my_imresize_NN(sub_sampled_img_my_imresize, 1/a, 'nn_interpolation');

  % Plots
  subplot(2,2,3), imshow(restored_img_imresize);
  title('Restored Image using imresize', 'Interpreter', 'none');
  subplot(2,2,4), imshow(restored_img_my_imresize);
  title('Restored Image using my_imresize_NN', 'Interpreter', 'none');
...
hold on;
stem(E_my_imresize);
legend('imresize', 'my_imresize_NN', 'Interpreter', 'none');
...
stem(G_imresize);
stem(G_my_imresize);
legend('original', 'imresize', 'my_imresize_NN', 'Interpreter', 'none');
