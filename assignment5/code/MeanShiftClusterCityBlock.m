function [clustCent,data2cluster,cluster2dataCell] = MeanShiftClusterCityBlock(dataPts,bandWidth,plotFlag)
...
while numInitPts
    ...
    while counterIter<15     %loop untill convergence
        
        sqDistToAll = sum(abs(repmat(myMean,1,numPts) - dataPts));    %dist squared from mean to all points still active
...
data2cluster = reshape(data2cluster,size(data2cluster,2),1);
